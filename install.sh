...
# Write a launcher that resolves the INSTALLED plugin at run time.
#
# settings.json holds a plain shell command, and Claude Code installs plugins
# into a version-numbered directory (.../requirements-framework/7.3.1) that
# changes on every update — so no path this installer could write stays valid.
# This shim is the stable indirection: it lives at a fixed path, carries no
# framework logic of its own, and therefore never goes stale. It reads the
# authoritative install record and hands over.
write_statusline_shim() {
    cat > "$SHIM_PATH" << 'SHIM'
#!/usr/bin/env bash
# Generated by the requirements-framework installer — do not edit; re-run
# ./install.sh to regenerate. Resolves the installed plugin and delegates.
set -u

registry="$HOME/.claude/plugins/installed_plugins.json"

# Entries can exist per scope (user, project); take the first whose statusline
# is actually on disk rather than trusting the record blindly.
#
# The search runs inside a command substitution on purpose. Looping over the
# jq output directly would bind THIS shell's stdin to that output, and the exec
# below would then hand jq's lines to the statusline instead of the JSON Claude
# Code piped in. read -r without IFS also keeps paths containing spaces whole.
root=$(jq -r '.plugins["requirements-framework@requirements-framework"][]?.installPath // empty' \
          "$registry" 2>/dev/null |
       while IFS= read -r candidate; do
           if [ -f "$candidate/statusline.sh" ]; then
               printf '%s' "$candidate"
               break
           fi
       done)
...
if [ -n "$root" ]; then
    exec bash "$root/statusline.sh" "$@"
fi

# Plugin not installed, or jq missing. Drain stdin and render nothing: a
# statusline that errors is worse than one that is briefly absent.
cat > /dev/null 2>&1
exit 0
SHIM
...
configure_statusline() {
    SHIM_PATH="$HOME/.claude/rf-statusline.sh"
    write_statusline_shim
    SHIM_PATH="$SHIM_PATH" python3 << 'PYTHON_SCRIPT'
import json
import os
import sys
...
try:
    ...
    # Register the phase-aware statusline, but never clobber a user's own —
    # only fill it in when absent, or when it still names a path a previous
    # version of this installer wrote.
    shim_path = os.environ.get("SHIM_PATH", "")
    if not shim_path:
        print("   ⚠️  SHIM_PATH unset — skipping statusline registration",
              file=sys.stderr)
        raise SystemExit(0)
    STATUSLINE_CMD = f"bash {shim_path}"

    # Both paths this installer used to write, and why neither survives:
    #   ~/.claude/plugins/requirements-framework/statusline.sh
    #       an unversioned plugin directory the plugin system no longer creates
    #   <repo>/plugins/requirements-framework/statusline.sh
    #       a working tree: renders whatever branch is checked out, and only
    #       exists on a machine that cloned the repo
    # Either resolves to nothing or to the wrong thing, silently. Move them.
    SUPERSEDED = (
        "/.claude/plugins/requirements-framework/statusline.sh",
        "/plugins/requirements-framework/statusline.sh",
    )
    ...
    elif existing_cmd.rstrip().endswith(SUPERSEDED):
        settings["statusLine"] = {"type": "command", "command": STATUSLINE_CMD}
        print("   ✅ Statusline repointed at the installed plugin")
    else:
