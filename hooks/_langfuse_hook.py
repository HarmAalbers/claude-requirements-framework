...
def _fail(msg: str) -> int:
    ...
    Mirrors hooks/langfuse-trace.py's _fail() — keep semantics in sync
...
def main() -> int:
    ...
    if not public_key or not secret_key:
        return _fail("tracing enabled but LANGFUSE_PUBLIC_KEY/SECRET_KEY unset")  # VENDOR-PATCH (d)
    ...
    try:
        ...
        return _fail(f"Langfuse client init failed: {e!r}")  # VENDOR-PATCH (d)
    ...
    try:
        with FileLock(LOCK_FILE):
            ...
            # emit turns
            emitted = 0
            emit_failures = 0  # VENDOR-PATCH (d)
            for t in turns:
                emitted += 1
                ...
                try:
                    ...
                    emit_failures += 1  # VENDOR-PATCH (d)
                    # continue emitting other turns

            ss.turn_count += emitted  # advance past failed turns too (no retry — accepted data-loss tradeoff; ADR-019)
            write_session_state(state, key, ss)
            save_state(state)

            if emit_failures:  # VENDOR-PATCH (d)
                return _fail(f"{emit_failures}/{emitted} turn(s) failed to emit — permanently skipped, no retry (see {LOG_FILE})")  # VENDOR-PATCH (d)
        ...
        return _fail(f"unexpected failure: {e!r}")  # VENDOR-PATCH (d)
