...
Ground truth lives **locally** in `golden_set/cases/*.json` (`reference_findings`, `reference_goal`), replayed via `scripts/run_eval.py`, and is **mirrored to the Langfuse dataset `golden-set`** by `scripts/sync_golden_set_to_langfuse.py` (item `id` = case id, `input` = agent + inlined diff, `expectedOutput` = findings + goal; items upsert by id, so re-runs are idempotent). Files are the source of truth — edit cases locally and re-sync; don't edit items in the UI. So for calibration:

- **Simple mode (default)**: run locally against the golden cases; no Langfuse dataset needed.
- **Langfuse experiment mode**: use the `golden-set` dataset (re-sync first if cases changed), then follow the experiment workflow below. Worth it when you want runs comparable in the Experiments UI.
