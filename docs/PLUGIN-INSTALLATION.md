...
- **14 lifecycle hooks** — registered via the plugin's `hooks/hooks.json`
...
All 14 lifecycle hooks are registered there via `${CLAUDE_PLUGIN_ROOT}`.
...
(agents/commands/skills). All 14 lifecycle hooks register from one file —
...
- **UserPromptSubmit / SubagentStart / PostToolUseFailure** — context
